...
public class LO_MyDmd {
    ...
    public LO_MyDmd() {
        ...
        form = new Form("Mes demandes", BoxLayout.y());
        ...
        form.getToolbar().addCommandToOverflowMenu("Mes demandes publiés", null, e3 -> {
            f4 = new Form("Mes demandes publiés", BoxLayout.y());
            list2 = ServiceLO.getMyPublishDmdObject();

            if (list2.size() == 0) {
                Container c1 = new Container(new FlowLayout(Component.CENTER));
                Label err = new Label("Pas des demandes publiées");
                c1.add(err);
                f4.add(c1);
            }
            for (LostObject t : list2) {
                ...
                l1.addPointerPressedListener(e -> {
                    ...
                    f2.getToolbar().addCommandToOverflowMenu("Modifier", null, e6 -> {
                        f3 = new Form("Modfier demandes", BoxLayout.y());
                        ...
                        modifier.addActionListener(m -> {
                            ...
                            Dialog.show("Modification effectué", "Votre demande a été modifié avec succees", "ok", null);
                            LO_MyAds myads = new LO_MyAds();
                            myads.getForm().show();
                        });
                    ...
                    f2.getToolbar().addCommandToOverflowMenu("Supprimer", null, e6 -> {
                        ...
                        Dialog.show("Suppression effectué", "Votre demande a été supprimé avec succees", "ok", null);
                        LO_MyAds myads = new LO_MyAds();
                        myads.getForm().show();
        ...
        form.getToolbar().addCommandToOverflowMenu("Mes demandes en attentes", null, e3 -> {
            f4 = new Form("Mes demandes en attentes", BoxLayout.y());
            list3 = ServiceLO.getMyWaitingDmdObject();

            if (list3.size() == 0) {
                Container c1 = new Container(new FlowLayout(Component.CENTER));
                Label err = new Label("Pas des demandes en attentes");
                c1.add(err);
                f4.add(c1);
            }
        ...
        form.getToolbar().addCommandToOverflowMenu("Mes demandes annulées", null, e3 -> {
            f4 = new Form("Mes demandes annulées", BoxLayout.y());
            list4 = ServiceLO.getMyCancelDmdObject();

            if (list4.size() == 0) {
                Container c1 = new Container(new FlowLayout(Component.CENTER));
                Label err = new Label("Pas des demandes annulées");
                c1.add(err);
                f4.add(c1);
            }
        ...
        for (LostObject t : list) {
            ...
            l1.addPointerPressedListener(e -> {
                ...
                f2.getToolbar().addCommandToOverflowMenu("Modifier", null, e3 -> {
                    f3 = new Form("Modfier demande", BoxLayout.y());
                    Label f3title = new Label("Titre :");
                    TextField txttitle = new TextField("", "Titre", 20, TextField.ANY);
                    txttitle.setText(t.getTitle());
                    Label f3description = new Label("Description :");
