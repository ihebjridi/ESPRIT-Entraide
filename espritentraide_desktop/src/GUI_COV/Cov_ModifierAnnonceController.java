 ...
 * @author HP
 */
public class Cov_ModifierAnnonceController implements Initializable {
    ...
    private void BtnMod_action(ActionEvent event) {
        ...
        Carpooling f = new Carpooling(title.getText(), description.getText(), "annonce",s.getId());
        CarpoolingService sv = new CarpoolingService();

        System.out.println("id annonce: " + Cov_MyAdsController.id_ads);
        sv.update1(Cov_MyAdsController.id_ads, f);

        try {
            ((Node) (event.getSource())).getScene().getWindow().hide();
            Parent root = javafx.fxml.FXMLLoader.load(getClass().getResource("/GUI_COV/Cov_MyAds.fxml"));
